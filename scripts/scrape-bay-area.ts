...
// Hard exclusion keywords in business name
const NAME_EXCLUDES = [
...
async function searchPlaces(query: string, city: string) {
  const res = await fetch("https://places.googleapis.com/v1/places:searchText", {
    ...
    headers: {
      ...
      "X-Goog-FieldMask": "places.id,places.displayName,places.formattedAddress,places.nationalPhoneNumber,places.websiteUri,places.rating,places.userRatingCount,places.types,places.shortFormattedAddress",
    },
    body: JSON.stringify({
...
async function main() {
  console.log("Searching Bay Area for golf simulator businesses...\n")

  for (const city of CITIES) {
    for (const query of QUERIES) {
      process.stdout.write(`  "${query}" in ${city}... `)
  ...
  // Now verify websites for simulator rental keywords
  console.log("Checking websites for mobile/rental/event simulator signals...\n")
  ...
  for (const p of kept) {
    process.stdout.write(`  ${p.displayName.text}... `)
    try {
      ...
      if (!res.ok) {
        console.log("SKIP (site unreachable)")
        continue
      }
  ...
  for (const p of verified) {
    ...
    if (existing) {
      console.log(`SKIP (exists): ${p.displayName.text}`)
      skipped++
      continue
    }
    ...
    await db.tool.create({
      data: {
        ...
        description: `${p.displayName.text} offers golf simulator experiences in the ${addr.city || "Bay Area"} area.`,
        tagline: `Golf simulator in ${addr.city || "Bay Area"}, ${addr.state || "CA"}`,
        content: "",
