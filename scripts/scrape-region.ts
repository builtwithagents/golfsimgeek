...
const CITIES = JSON.parse(process.env.CITIES || "[]") as string[]
const REGION_NAME = process.env.REGION_NAME || "region"
...
function shouldExclude(place: Place): string | null {
  ...
  for (const kw of NAME_EXCLUDES) { if (name.includes(kw)) return `Name "${kw}"` }
  for (const type of TYPE_EXCLUDES) { if (place.types.includes(type)) return `Type "${type}"` }
...
function parseAddress(formatted: string) {
  ...
  return { address: parts[0] || "", city: parts.length >= 3 ? parts[parts.length - 2] : "", state: stateZip?.[1] || "", zip: stateZip?.[2] || "" }
}
...
async function main() {
  console.log(`Searching ${REGION_NAME} for golf simulator businesses...\n`)
  let searchCount = 0
  ...
  for (const city of CITIES) {
    for (const query of QUERIES) {
      ...
      let newCount = 0
      for (const p of places) { if (!seen.has(p.id)) { seen.add(p.id); allPlaces.push(p); newCount++ } }
  ...
  console.log(`\nTotal unique: ${allPlaces.length}\nFiltering...\n`)
  const kept: Place[] = []
  for (const p of allPlaces) {
    const reason = shouldExclude(p)
    if (reason) { console.log(`  EXCLUDED: ${p.displayName.text} — ${reason}`) }
    else { console.log(`  KEPT: ${p.displayName.text}`); kept.push(p) }
  }

  console.log(`\nKept ${kept.length}/${allPlaces.length}\nChecking websites...\n`)
  ...
  for (const p of kept) {
    ...
    try {
      ...
      const res = await fetch(p.websiteUri!, { signal: controller.signal, headers: { "User-Agent": "Mozilla/5.0 (compatible; GolfSimGeek/1.0)" }, redirect: "follow" })
      clearTimeout(timeout)
      if (!res.ok) { console.log("SKIP"); continue }
      const text = (await res.text()).toLowerCase().replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").slice(0, 50000)
      let score = 0; const matches: string[] = []
      for (const kw of MOBILE_KEYWORDS) { if (text.includes(kw)) { score++; matches.push(kw) } }
      if (score >= 2) { console.log(`YES (${score}) — ${matches.slice(0, 3).join(", ")}`); verified.push({ ...p, score }) }
  ...
  const cat = await db.category.findFirst({ where: { slug: "mobile-rentals" } }) || await db.category.create({ data: { name: "Mobile Rentals", slug: "mobile-rentals" } })
  ...
  for (const p of verified) {
    ...
    if (existing) { console.log(`SKIP: ${p.displayName.text}`); skipped++; continue }
    const addr = parseAddress(p.formattedAddress)
    try {
    await db.tool.create({
      data: {
        name: p.displayName.text, slug, websiteUrl: p.websiteUri!,
        description: `${p.displayName.text} offers golf simulator experiences in the ${addr.city || REGION_NAME} area.`,
        tagline: `Golf simulator in ${addr.city || REGION_NAME}, ${addr.state}`,
        content: "", submitterEmail: admin.email, owner: { connect: { id: admin.id } },
        phone: p.nationalPhoneNumber || null, address: addr.address, city: addr.city,
        state: STATE_NAMES[addr.state] || addr.state, stateCode: addr.state, zipCode: addr.zip, placeId: p.id,
        googleRating: p.rating || null, reviewCount: p.userRatingCount || 0,
        mobileConfirmed: false, status: ToolStatus.Published, publishedAt: new Date(),
    ...
    added++
      ...
      if (err.code === 'P2002') { console.log(`SKIP (duplicate): ${p.displayName.text}`); skipped++ }
      else throw err
    }
  }

  console.log(`\nDone! Added: ${added}, Skipped: ${skipped}`)
  const newListings = await db.tool.findMany({ where: { content: "", status: "Published" }, select: { id: true, name: true, websiteUrl: true, city: true, stateCode: true } })
  console.log("\n=== NEEDING ENRICHMENT ===")
